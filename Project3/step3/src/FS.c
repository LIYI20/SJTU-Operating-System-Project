...
//user table
int user_count = 0;
char user_table[100][100];//存用户名称

char response[10000];//返回信息，因为user指令会调用内部函数，不能让内部函数也send，所以用response在外部来传输
...
int cmd_f(tcp_buffer *write_buf, char *args, int len){
    ...
    // {
    //     if (write_inode_to_disk(&inode_table[i], i,client) < 0)
    //     {
    ...
    // }
    format = true;
    strcpy(response, "Format is done");
...
int cmd_mk(tcp_buffer *write_buf, char *args, int len)
{
    ...
    if (dir_search(&inode_table[cur_dir], name,0)>=0)
    {
        ...
        strcpy(response, "The name has existed");
...
int cmd_mkdir(tcp_buffer *write_buf, char *args, int len)
{
    ...
    { // 重名
        ...
        strcpy(response, "Directory name has existed");
    ...
    else
    {
        ...
        strcpy(response, "mkdir successful");
...
int cmd_cd(tcp_buffer *write_buf, char *args, int len)
{
    ...
    char tmp[100];
    sprintf(tmp,"cur_dir:%d  cur_dir_str:%s", cur_dir, cur_dir_str);
    printf("%s\n", tmp);
    strcpy(response, tmp);
...
int cmd_rmdir(tcp_buffer *write_buf, char *args, int len)
{
    ...
    else
    {
        ...
        strcpy(response, "rmdir successful");
...
int cmd_cat(tcp_buffer *write_buf, char *args, int len)
{
    ...
    if (read_file(&inode_table[id], buf) == 0)
    {
        ...
        strcpy(response, "Can't catch the file");
...
int cmd_w(tcp_buffer *write_buf, char *args, int length)
{
    ...
    if (write_file(&inode_table[id], data) == 0){
        ...
        strcpy(response, "Write data successful");
    ...
    else
    {
        ...
        strcpy(response, "Write data failed");
...
int cmd_i(tcp_buffer *write_buf, char *args, int length)
{
    ...
    if (write_file(&inode_table[id], buf) == 0){
        ...
        strcpy(response, "Insert data successful");
    ...
    else
    {
        ...
        strcpy(response, "Insert data failed");
...
int cmd_d(tcp_buffer *write_buf, char *args, int length)
{
    ...
    if (pos > file_size)
    {
        ...
        strcpy(response, "Pos can't larger than file_size");
...
{
    //在退出的时候要将cur_dir和cur_dir_str恢复到root
    cur_dir = 0;
    strcpy(cur_dir_str, "/");
    strcpy(response, "Bye!");
...
void add_client(int id)
{
    ...
    //当连接一个新用户的时候，要给新用户提供不同的根目录
    //OK,我没有用这个函数，因为我准备传用户名更直观一点，用id不直观，所以我又造了一个新指令
...
int main(int argc, char *argv[])
{
    ...
    printf("Connected to BDS server\n");
    // 调用一下f指令初始化一下,文件系统自动初始化
    cmd_f(0," ", 0);
